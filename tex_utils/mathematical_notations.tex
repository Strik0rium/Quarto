...
\begin{itemize}
    ...
    \item $\mathrm{i}$ 表示虚单位, 它满足 $\mathrm{i}^2 = -1$, 在工程学中记作 $\mathrm{j}$. 由于本书中含有较多的来自工程学问题的案例, 因此我们统一使用 $\mathrm{j}$ 表示虚数单位.
    \item $\pi$ 表示圆周率.
...
\begin{itemize}
    ...
    \item $u(t)$ 表示单位阶跃函数, $u[n]$ 表示单位阶跃序列.
    \item $\delta(t)$ 表示单位冲激函数, $\delta[n]$ 表示单位冲激序列.
    \item $h(t)$ 表示系统的冲激响应, $h[n]$ 表示系统的冲激响应序列.
    \item $y(t) = h(t) * x(t)$ 表示系统的卷积, $y[n] = h[n] * x[n]$ 表示系统的卷积序列.
    \item 信号 $x(t)$ 的 Fourier 变换记作 $X(\jm\omega)$, Laplace 变换记作 $X(s)$, 其中 $s = \sigma + \jm\omega$ 是复数.
    \item 信号 $x[n]$ 的 $z$-变换记作 $X(z)$, 其中 $z = r\mathrm{e}^{\jm\omega}$ 是复数.
    \item 线性时不变系统 $H$ 的传递函数 (系统函数) 记作 $H(\jm\omega)$ 或者 $H(s)$.
\end{itemize}
